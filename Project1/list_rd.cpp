...
//链表插入操作，插入 num 位置，插入 1 位置表示插入链表头 
void List::Insert(int num, int x)
{
	...
	if (!head && num == 1) {	//空链表，插入第一个结点，需要创建
		head = new ListNode;
		head->data = x;
		head->next = nullptr;
	}
...
void List::Delete_pos(int num)	//删除第 num 的结点
{
	if (!head) {
		...
		if (!p || num < 1) {	//不是 !p->next 是因为如果只有一个结点那它也会变成位置不合法
			cout << "位置不合法" << endl;
		}
		else if (num == 1) {		//删除头结点，不单独写会与下面的情况重合
			head = head->next;
			delete p;
		}
...
void List::sort_value(int x)
{
	...
	while (head_temp) {
		if ((!isDelete) && (head_temp->data == x)) {	//如果还未删去，碰到x时不赋值给链表
			...
			ListNode* d = head_temp;
			Insert(1, head_temp->data);	//分类好的链头依次插入表头，顺序为反
			head_temp = head_temp->next;
			delete d;
		}
	}
